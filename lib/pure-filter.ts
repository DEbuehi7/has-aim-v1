...
export async function callClaude(userMessage: string, options: PureFilterOptions = {}): Promise<string> {
...
export function validateOutput(text: string): string {
  const forbidden = ["I hope this helps","feel free","certainly","great question","as per","going forward","utilize","delve","straightforward","empower"];
  let cleaned = text;
  forbidden.forEach(p => { cleaned = cleaned.replace(new RegExp(p, "gi"), ""); });
  return cleaned.trim();
}
